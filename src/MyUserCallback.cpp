...
// MyUserCallback::MyUserCallback(IloEnv env, const IloBoolVarArray &ref_l, Data &d, int source, int sink) : IloCplex::UserCutCallbackI(env), d(d)
MyUserCallback::MyUserCallback(IloEnv env, const IloBoolVarArray &ref_l, Data &d, int source, int sink) : IloCplex::UserCutCallbackI(env), d(d)
{
  vars_l = ref_l;
  _source = source;
...
IloCplex::CallbackI *MyUserCallback::duplicateCallback() const
{
  return new (getEnv()) MyUserCallback(getEnv(), vars_l, d, _source, _sink);
}

void MyUserCallback::main()
{
  IloEnv env = getEnv();
